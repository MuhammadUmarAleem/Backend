  ...
  connection.query(checkUserQuery, [Email, Id], (err, result) => {
    ...
    connection.query(updateUserQuery, [Email, FirstName, LastName, Id], (err, userResult) => {
      ...
      if(!file){
        ...
        UPDATE Employees
        SET Position = ?,  CNIC = ?, HomeAddress = ?, GitHubUsername = ?, BankAccountName = ?, BankAccountNo = ?
        WHERE UserId = ?;
      `;
        connection.query(updateEmployeeQuery, [Position,  CNIC, HomeAddress, GitHubUsername, BankAccountName, BankAccountNo,Id], (err, empResult) => {
          if (err) {
        ...
        const updateEmployeeQuery = `
        UPDATE Employees
        SET Position = ?,  CNIC = ?, HomeAddress = ?, GitHubUsername = ?, BankAccountName = ?, BankAccountNo = ?, ProfileImage = ?
        WHERE UserId = ?;
      `;
        connection.query(updateEmployeeQuery, [Position,  CNIC, HomeAddress, GitHubUsername, BankAccountName, BankAccountNo, ProfileImage, Id], (err, empResult) => {
          if (err) {
            // Rollback the user update if employee update fails
            connection.query(updateUserQuery, [req.body.Email, req.body.FirstName, req.body.LastName, req.body.PhoneNo, req.body.Id], (err) => {
              if (err) {
