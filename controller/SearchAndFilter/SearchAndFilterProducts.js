...
// - `availability` (string): `accept_order`, `available_now`, `available_later`.
// - `tags` (string): Comma-separated tags (e.g., `electronics,wireless`).
// - `sort` (string): Sorting option (e.g., `price_asc`, `price_desc`, `rating_desc`, `newest`).
...
exports.SearchAndFilterProducts = async (req, res) => {
  const {
    ...
    page = 1,
    limit = 12,
    sort = 'newest' // Default sorting option
  } = req.query;
  ...
  try {
    ...
    if (sort === 'price_asc') {
      ...
      sortOptions.createdAt = -1; // Assuming products have a createdAt field
    }

    // Find products with the applied filters and sorting
